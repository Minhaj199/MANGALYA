...
export const UserSearchPage = () => {
  ...
  if(ageRange.min===''&&ageRange.max===''&&selectedDistrict===''&&!selectedInterests?.length){
    ...
    if (ageRange.min&&ageRange.max&& parseInt (ageRange.min)> parseInt (ageRange.max)||
    parseInt (ageRange.min)<18||parseInt (ageRange.min)>60||parseInt (ageRange.max)<18||parseInt (ageRange.max)>60) {
          showToast(
            "Please provide a valid age range",
            "warning"
          );
          console.log(ageRange.min,ageRange.max)
          return;
        }
  ...
  return (
    ...
    <div className=" max-w-2xl mx-auto p-6 border  bg-white shadow-2xl shadow-orange-300 rounded-lg">
      ...
      {(ageRange.min || ageRange.max || selectedDistrict || selectedInterests.length > 0) && (
        <div className="mt-4 p-3 bg-gray-100 rounded-lg">
          <h2 className="text-sm font-semibold mb-2">Current Filters:</h2>
          <div className="flex flex-wrap gap-2">
            {ageRange.min && <span className="bg-blue-100 px-2 py-1 rounded-full text-xs">Min Age: {ageRange.min}</span>}
            {ageRange.max && <span className="bg-blue-100 px-2 py-1 rounded-full text-xs">Max Age: {ageRange.max}</span>}
            {selectedDistrict && <span className="bg-green-100 px-2 py-1 rounded-full text-xs">District: {selectedDistrict}</span>}
            {selectedInterests.map(interest => (
              <span key={interest} className="bg-purple-100 px-2 py-1 rounded-full text-xs">
                {interest}
              </span>
            ))}
          </div>
