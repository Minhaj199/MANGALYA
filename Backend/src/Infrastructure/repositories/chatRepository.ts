...
import { ChatRoom, FetchedChat, Member } from "../../types/TypesAndInterfaces"
    ...
    async fetchDatasForCount(id: string): Promise<{ chats: { _id: string; } }[]|[]> {
      try {
        const response:{chats:{_id:string}}[]|[]=await chatModel.aggregate(
          ...
          ,{$project:{_id:0,chats:1}},{$unwind:'$chats'},{$match:{$and:[{'chats.viewedOnNav':false},{'chats.viewedList':false},{'chats.senderId':new Types.ObjectId(id)}]}},{$project:{'chats._id':1}}]
      )
        return response
